...
const { loginPerson} = require("./roomController");
const { signinPerson, roomData } = require("./calendarController");
const { signinUser} = require("./cardController");
...
const postSignIn = function (req, res) {
  ...
  con.query(sql, [email, password], function (err, data, fields) {
    if (data.length > 0) {
      ...
      var calendarAttendees = email.replace("@gmail.com", "");
      roomData(calendarAttendees);
      attendeesData = calendarAttendees;
      res.redirect("/rooms");
    } else {
...
function sendEmail(email, token) {
  ...
  var mail = nodemailer.createTransport({
    service: "gmail",
    auth: {
      user: "meetingroombooking12", // Your email id
      pass: "Aishu@107", // Your password
    },
  });
  var mailOptions = {
    ...
    html:
      `<p>Hi <b>${email.replace("@gmail.com", "")},</b> </p>` +
